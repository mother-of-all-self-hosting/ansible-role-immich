...
#
# Tags look like `v<Immich version>-<release>`:
#
# - if defaults/main.yml points at a Immich version that has never been
#   released, the release counter restarts at 0 (`v3.1.1-0`)
# - otherwise the counter is incremented (`v3.1.1-1`), but only if something
...
version="$(sed -nE 's|^immich_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Immich version from $defaults_path"
	exit 1
fi
